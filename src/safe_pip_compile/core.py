...
def _sanitize_compile_output(
    ...
    # Remove the command header block:
    # # by the following command:
    # #
    # #    pip-compile ... / safe-pip-compile ...
    # #
    i = 0
    while i < len(lines):
        if "# by the following command:" in lines[i]:
            if (
                i + 3 < len(lines)
                and lines[i + 1].strip() == "#"
                and (
                    "#    pip-compile" in lines[i + 2]
                    or "#    safe-pip-compile" in lines[i + 2]
                )
            ):
                del lines[i : i + 3]
                changed = True
    ...
    for line in lines:
        ...
        new_line = line
        if "This file is autogenerated by pip-compile" in new_line:
            new_line = new_line.replace("by pip-compile", "by safe-pip-compile")
